...
try:
    ...
    from .utils import load_class_names, preprocess_face
except ImportError:
    # Allow running as a script: `python src/infer_cam.py` or from within src.
...
def detect_faces(net, frame_bgr):
...
def main(cam_index=0):
    ...
    if not CLASS_PATH.exists():
        raise SystemExit(f"Class names not found: {CLASS_PATH} (produced by training)")
